...
// Game
const mainpuppy              = dom("main_puppy");
...
const elpuppyCount           = dom("puppy_Count");
const elClickSpeed            = dom("click_speed");
const elCPC                   = dom("cpc");
const elCPS                   = dom("cps");
const elCashCount             = dom("cash_count");
const elGoldenpuppyCount     = dom("golden_puppy_count");
...
const elEnablepuppySounds = dom('enablepuppySounds');
...
class Player{
    constructor(data_version,puppys,cpc,cps,equippedHoes,cash,
        clickSpeedRecord,fallingConsecRecord,trinket_completion,pages,golden_puppys,
        ...
        this.pages=pages;
        this.golden_puppys=golden_puppys;
...
class statsTracker{
    constructor(puppys,click_puppys,idle_puppys,bonus_puppys,cash,falling_puppys_grabbed,golden_puppys,prestige_count,clicks,hoes,tomes_bought,trinkets_complete,boosts_used) {
        this.puppys=puppys;
        this.click_puppys=click_puppys;
        this.idle_puppys= idle_puppys;
        this.bonus_puppys=bonus_puppys;
        this.cash=cash;
        this.falling_puppys_grabbed=falling_puppys_grabbed;
        this.golden_puppys=golden_puppys;
...
const default_player = new Player(
    ...
    0, //puppys
    1, //cpc; puppys per click
    0, //cps; puppys per Second
    0, //equippedHoes
    0, //cash
    0, //clickSpeedRecord
...
document.addEventListener('DOMContentLoaded', function() {
    if (!localStorage.getItem('rewardBalance')) {
      localStorage.setItem('rewardBalance', '0');
    }
    if (!localStorage.getItem('lastClaimDate')) {
  ...
  function startTask(taskId, taskUrl) {
    ...
    let taskButton = document.querySelector(`#${taskId} button`);
    taskButton.disabled = true;
    taskButton.textContent = 'Checking...';
  
    window.open(taskUrl, '_blank');
  
    setTimeout(() => {
      taskButton.textContent = 'Complete';
      taskButton.classList.add('claim');
      taskButton.disabled = false;
      taskButton.onclick = () => markTaskComplete(taskId);
    }, 10000);
  }
  ...
  function checkTasksCompletion() {
    ...
    let taskIds = ['task-yt-sub', 'task-yt-watch', 'task-twitter', 'task-tiktok'];
    ...
    document.getElementById('claim-reward').disabled = !allTasksCompleted;
  }
  ...
  function claimReward() {
    ...
    if (lastClaimDate === today) {
      alert('You can only claim the reward once a day.');
      return;
    }
  
    let rewardBalance = parseInt(localStorage.getItem('rewardBalance'));
    rewardBalance += 2000; // Assuming 10 points per task, and there are 4 tasks
    localStorage.setItem('rewardBalance', rewardBalance.toString());
  ...
  function updateRewardBalance() {
    document.getElementById('reward-balance').textContent = localStorage.getItem('rewardBalance');
  }
...
class Tome {
    ...
    add(amount=1){
        ...
        if(player.golden_puppys < price) return; 

        // Updates values
        player.golden_puppys -= price;
        this.value += amount;
...
const Default_Carl = {
    ...
    shop: {
        ...
        cosmetic: {
            ...
            // Farmable
            'farmable/pixel_puppy': new carlListing(8),
            'farmable/pixel_golden_puppy': new carlListing(10),
            ...
            // puppy variants
            'farmable/alien_puppy': new carlListing(3),
            'farmable/demon_puppy': new carlListing(3),
            'farmable/ghost_puppy': new carlListing(3),
            'farmable/rainbow_puppy': new carlListing(3),
...
function hireJared() {
    ...
    if(player.puppys < hireCost) return toast(...toasts.error_jared_hire_cost); // Too expensive
    player.puppys -= hireCost;
...
function optionSoundsDisable(state) {
    ...
    elEnablepuppySounds.disabled = !state;
    if(!state) stopMusic(); // puppy sounds
}
...
const default_settings = {
    ...
    enablepuppySounds: false,  // boolean
...
// Example of detecting changes and sending data via AJAX
document.getElementById('puppy_Count').addEventListener('DOMSubtreeModified', function() {
    let newCount = this.innerText;

    // Send the new count to the server
    let xhr = new XMLHttpRequest();
    xhr.open("POST", "update_count.php", true);
    xhr.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
    xhr.send("puppy_Count=" + newCount);
});


/** Earn puppys function
 * @param {number} amount Amount of $Paw to be earned
 ...
 */
function earnpuppys(amount, type, useMousePos = false) {
    player.puppys += amount; // Grant $Paw

    // Statistics
    player.prestige.puppys += amount;
    player.lifetime.puppys += amount;
    player.prestige[`${type}_puppys`] += amount;
    player.lifetime[`${type}_puppys`] += amount;
...
// puppy event listeners
var hold = {
    active: false,
    delay:  undefined,
    clock:  undefined,
}
mainpuppy.addEventListener('mousedown',  e => { holdStart(); });
mainpuppy.addEventListener('touchstart', e => { e.stopImmediatePropagation(); holdStart(); });
/** Holding down clicks Paw
...
function holdStart(useMousePos = true, method=0) {
    ...
    hold.delay = setTimeout(() => {
        ...
        hold.clock = setInterval(() => {
            ...
            puppyClick(false, useMousePos, method);
...
mainpuppy.addEventListener('mouseup', () => { holdStop(); });
mainpuppy.addEventListener('mouseout', event => {
    if(!hold.active || clickMethod === 1) return;
    let obstruct = event.relatedTarget; // Don't stop if moused out to falling puppy
    if(obstruct.classList.contains('falling_puppy')) return obstruct.click();
    holdStop();
});
mainpuppy.addEventListener('touchend', () => { holdStop(); });
mainpuppy.addEventListener('touchcancel', () => { holdStop(); });
...
// puppy click
mainpuppy.addEventListener('touchstart', event => { puppyClick(event, true, 2) });
mainpuppy.addEventListener('click', event => { puppyClick(event) });
 ...
 * @returns 
 */
function puppyClick(event, useMousePos=true, source=0) {
    ...
    mouseConfetti([0, 2], ccpuppy, 200, 5);
    clickSpeedHandler(true); // Click speed
    fallingpuppy(); // Falling Paw chance

    if(settings.enablepuppySounds) randomSound('crunch', 95); // Sound effect
}

// Falling puppys
var fallingpuppyPromiser = 0;
var fallingID = 0;
var fallingConsecutive = 0;
const fallingpuppysArea = dom('fallingpuppysArea');
/** Creates a falling puppy */
function fallingpuppy() {
    ...
    if(type === 'puppy') {
        ...
        if(player.lifetime.puppys >= 1000000000000000)   { min = 12; max = 12; }
        else if(player.lifetime.puppys >= 1000000000000) { min =  8; }
        else if(player.lifetime.puppys >= 1000000000)    { max =  8; }
    ...
    let image = type === 'puppy' ? cosmetics.farmable[settings.cosmetics.farmable].image
        : type === 'cash' ? './assets/cash.png' : './assets/boosts/falling.png';
    element.setAttribute('src', image);
    element.setAttribute('draggable', "false");
    element.classList.add('falling_puppy');
    ...
    let catchfunc = () => { catchpuppy(element.id, type, amount); };
    ...
    // To page
    fallingpuppysArea.append(element);
    let time = type === 'boost' ? 6000 : 2600;
    setTimeout(() => { if(dom(element.id) !== null) dom(element.id).remove(); }, time);

    /** Grant reward when falling puppy is caught */
    function catchpuppy(id, type, amount) {
        dom(id).remove();
        fallingConsecutive++;
        ...
        if(type === 'puppy') {
            ...
            player.prestige.falling_puppys_grabbed++;
            player.lifetime.falling_puppys_grabbed++;
            earnpuppys(amount, 'bonus', true);
            mouseConfetti([6, 8], ccpuppy, 300);
        } else if(type === 'cash') {
...
/** Updates puppy count on the page */
function puppyCount() {
    count = settings.full_numbers !== true ? DisplayRounded(Math.floor(player.puppys), 3, 1000000) : numCommas(Math.floor(player.puppys));
    eInnerText(elpuppyCount, count);
}
...
function characterButtons() {
    for(i in levelable) {
        ...
        let state  = (player.puppys < character.lvlupPrice);
        if(mbsel != 0) state = (player.puppys < getLevelPrice(character, character.lvl, multibuy[mbsel]).toFixed(0));
...
function updateCharlesShop() {
    ...
    style(elCharles.shop.improveWorkingConditions, 'cant_afford', (Charles.improveWorkingConditions.priceQuery(multibuy[mbsel]) >= player.golden_puppys));
    style(elCharles.shop.betterHoes, 'cant_afford', (Charles.betterHoes.priceQuery(multibuy[mbsel]) >= player.golden_puppys));
    style(elCharles.shop.decreaseWages, 'cant_afford', (Charles.decreaseWages.priceQuery(multibuy[mbsel]) >= player.golden_puppys));

    // Update tome prices
    if(Charles.improveWorkingConditions.priceQuery(multibuy[mbsel])>99999){
        eInnerText(elCharles.prices.improveWorkingConditions, `${DisplayRounded(Charles.improveWorkingConditions.priceQuery(multibuy[mbsel]),2)} Golden puppys`);
    }else{
        eInnerText(elCharles.prices.improveWorkingConditions, `${numCommas(Charles.improveWorkingConditions.priceQuery(multibuy[mbsel]))} Golden puppys`);
    }
    if(Charles.betterHoes.priceQuery(multibuy[mbsel])>99999){
        eInnerText(elCharles.prices.betterHoes, `${DisplayRounded(Charles.betterHoes.priceQuery(multibuy[mbsel]),2)} Golden puppys`);
    }else{
        eInnerText(elCharles.prices.betterHoes, `${numCommas(Charles.betterHoes.priceQuery(multibuy[mbsel]))} Golden puppys`);
    }
    if(Charles.decreaseWages.priceQuery(multibuy[mbsel])>99999){
        eInnerText(elCharles.prices.decreaseWages, `${DisplayRounded(Charles.decreaseWages.priceQuery(multibuy[mbsel]),2)} Golden puppys`);
    }else{
        eInnerText(elCharles.prices.decreaseWages, `${numCommas(Charles.decreaseWages.priceQuery(multibuy[mbsel]))} Golden puppys`);
    }
    ...
    style(dom('charles_box'), 'glowing', (
        Charles.improveWorkingConditions.price <= player.golden_puppys
        || Charles.betterHoes.price <= player.golden_puppys
        || Charles.decreaseWages.price <= player.golden_puppys
    ));
...
/** Updates golden puppy count on the page. Won't run if prestige_available is false */
function updateGC() {
    if(player.lifetime.prestige_count < 1 && player.golden_puppys < 1) return;
    eInnerText(elGoldenpuppyCount, DisplayRounded(player.golden_puppys, 2));
}
...
function updateMainIcon() {
    ...
    // Golden puppy
        ...
        elMainIcon.src   = './assets/theme/pixel_golden_puppy.png';
        elMainIcon.title = 'Prestiged';
    }
    // Default
    else {
        elMainIcon.src   = './assets/pixel_puppy_32x.png';
        elMainIcon.title = 'puppy Clicker';
    }
}
...
function updatePrestigeMenu() {
    eInnerText(elPrestigeMenuGCCount, DisplayRounded(player.golden_puppys, 2));
...
/** puppys per click */
var cpsbuff = 0;
var cpsInterval = setInterval(puppysPerSecond, 100);
function puppysPerSecond() {
    let cps = player.cps;
    if(cps <= 0) return;
    if(cpsbuff != 0) {
...
/** Performs a prestige, which gives Golden puppys and resets attributes
...
function prestige() {
    ...
    // Give golden puppys
    player.golden_puppys += player.prestige_potential;
    player.lifetime.golden_puppys += player.prestige_potential;
    ...
    player.puppys = 0;
    cpsInterval = setInterval(puppysPerSecond, 100);
    tips.tracker=0;
...
/** Calculates Paw Per Click or Per Second Based on inputing Bill or Belle
 * @param {Object} character 
 * @returns puppys per Click or puppys per Second
 */
 function calculatepuppys(character) {
...
function recalculatepuppysPer() {
    player.cpc = calculatepuppys(Boomer_Bill);
    player.cps = calculatepuppys(Belle_Boomerette);
    calculatePrestigePotential();
    updateCPC();
}
...
function calculatePrestigePotential() {
    ...
    if(
        (player.lifetime.golden_puppys === 0 && player.prestige_potential >= 20)
        || (player.lifetime.golden_puppys != 0 && player.prestige_potential > player.lifetime.golden_puppys)
...
function createTool(type=0, amount=1, progress=0, intended_character=false) {
    ...
    // Checks if player has enough puppys
    let price = toolCost(type, amount);
    if(price * ((Jared.data.greg_min_start.value || 100) / 100) > player.puppys && progress === 0) return //toast("Too expensive!", "That tool is currently too expensive", '', false, true);
...
function updateAllTools() {
    ...
    function updateTool(character, type) {
        ...
        // Greg
            ...
            if(gregLevelTest(type) && toolCost(type, multibuy[mbsel]) * ((Jared.data.greg_min_start.value / 100) || 1) <= player.puppys) {
...
function seeButton(button='prestige') {
    if(button === 'prestige') {
        ...
        dom('prestige_menu_button_img').src = `./assets/icons/pixel_puppy_white.png`; 
...
const statsNumbers = {
    // Prestige
    prestige_puppys:            dom('prestige_puppys'),
    prestige_puppys_clicked:    dom('prestige_puppys_clicked'),
    prestige_puppys_idled:      dom('prestige_puppys_idled'),
    prestige_puppys_bonus:      dom('prestige_puppys_bonus'),
    prestige_clicks:             dom('prestige_clicks'),
    prestige_falling_puppys_grabbed: dom('prestige_falling_puppys_grabbed'),
    ...
    lifetime_puppys:                 dom('lifetime_puppys'),
    lifetime_puppys_clicked:         dom('lifetime_puppys_clicked'),
    lifetime_puppys_idled:           dom('lifetime_puppys_idled'),
    lifetime_puppys_bonus:           dom('lifetime_puppys_bonus'),
    lifetime_golden_puppys:          dom('lifetime_golden_puppys'),
    lifetime_golden_puppys_spent:    dom('lifetime_golden_puppys_spent'),
    ...
    lifetime_falling_puppys_grabbed: dom('lifetime_falling_puppys_grabbed'),
...
function loadStatistics() {
    // Prestige
    statsNumbers.prestige_puppys.innerText                 = DisplayRounded(player.prestige.puppys.toFixed(0));
    statsNumbers.prestige_puppys_clicked.innerText         = DisplayRounded(player.prestige.click_puppys.toFixed(0));
    statsNumbers.prestige_puppys_idled.innerText           = DisplayRounded(player.prestige.idle_puppys.toFixed(0));
    statsNumbers.prestige_puppys_bonus.innerText           = DisplayRounded(player.prestige.bonus_puppys.toFixed(0));
    statsNumbers.prestige_clicks.innerText                  = numCommas(player.prestige.clicks);
    statsNumbers.prestige_falling_puppys_grabbed.innerText = numCommas(player.prestige.falling_puppys_grabbed);
    ...
    statsNumbers.lifetime_puppys.innerText                 = DisplayRounded(player.lifetime.puppys.toFixed(0));
    statsNumbers.lifetime_puppys_clicked.innerText         = DisplayRounded(player.lifetime.click_puppys.toFixed(0)) ;
    statsNumbers.lifetime_puppys_idled.innerText           = DisplayRounded(player.lifetime.idle_puppys.toFixed(0));
    statsNumbers.lifetime_puppys_bonus.innerText           = DisplayRounded(player.lifetime.bonus_puppys.toFixed(0));

    statsNumbers.lifetime_golden_puppys.innerText          = numCommas(player.lifetime.golden_puppys);
    statsNumbers.lifetime_golden_puppys_spent.innerText    = numCommas(player.lifetime.golden_puppys - player.golden_puppys);
    ...
    statsNumbers.lifetime_falling_puppys_grabbed.innerText = numCommas(player.lifetime.falling_puppys_grabbed);
...
function tipchange() {
    ...
    if(player.equippedHoes > 0 || player.prestige.puppys > 100000 && tips.tracker === 0) tips.tracker = 1; // 1 tool equipped or 100k
    else if(player.prestige.puppys > 1000000 && tips.tracker === 1) tips.tracker = 2; // 1 million
    else if(player.prestige.puppys > 1000000000 && tips.tracker === 2) tips.tracker = 3; // 1 billion
